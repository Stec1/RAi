...
|---|---|---|---|
| W-01 | `apps/web/src/components/topology/TopologyGraph3D.tsx` | EVOLVE @R-01, @R-03…R-05 | R-01 interim: remove domain nodes/edges and the domain-cone placement; observatories get deterministic name-hash shell positions and their VisualSignature primary color, so Explore never breaks between R-01 and R-03. R-03: full-screen meta-graph + new spatial model (Design round A). R-04 depth, R-05 life. |
| W-02 | `apps/web/src/components/topology/topology-layout.ts` | DEMOLISH @R-01 | Dead code from the pre-PP-05 SVG topology (its own header says "for the /explore SVG topology"). Only `DomainSeed` + `domainColor` are still imported — both domain concepts that die with domains. Earliest safe step: R-01. |
| W-03 | `apps/web/src/components/topology/ExploreInfoPanel.tsx` (+ `.module.css`) | DEMOLISH @R-03 | This IS the docked Inspector (imported by RaiTerminal). Replaced by the floating selected-node card. R-01 interim: strip domain rows only. |
| W-04 | `apps/web/src/components/terminal/RegistryRail.tsx` | DEMOLISH @R-03 | Terminal side column. R-01 interim: remove domain sections, keep a flat world list. |
...
| W-08 | `apps/web/src/components/terminal/RaiTerminal.tsx` | EVOLVE @R-01, @R-03 | R-01: drop the domain fetch + domain wiring. R-03: recomposed as the full-screen graph shell (side columns gone). |
...
| W-13 | `apps/web/src/components/dashboard/DashboardScreen.tsx` | EVOLVE @R-01, @R-08 | R-01: domain pills out, `visibility` control in. R-08: simplified to the v2 owner surface. R-09: credits/plan display. |
| W-14 | `apps/web/src/data/mock-observatories.ts` | EVOLVE @R-01 | Becomes RA's own first worlds (Wawel, Signal Garden — R-DL-03), recomposed under the new content model in Phase 1. The `kind` virtual/real values, `domainSlug`, and fixed layout offsets die here. |
| W-15 | `apps/web/src/lib/universe-observatories.ts` | EVOLVE @R-01 | DTO drops `domainIds`, gains `visibility`; the real+mock merge becomes real + RA's first worlds. |
| W-16 | `apps/web/src/lib/topology-types.ts` | EVOLVE @R-01 | Remove `DomainDTO` and domain-shaped types; keep selection/view-command types. |
...
| W-27 | `/@name` public world route | CREATE @R-01 | New top-level dynamic segment (or middleware rewrite) accepting the `@`-prefixed handle; engineering shape decided at R-01. |
| W-28 | `@react-three/drei` + `@react-three/fiber` in `apps/web/package.json` | DEMOLISH @R-01 | **Dead dependencies** — declared but imported nowhere (verified by grep). Earliest safe step: R-01 (first code step; touches lockfile). |

## API

| ID | Path | Tag | Interim / notes |
|---|---|---|---|
| A-01 | `apps/api/src/routes/domains.ts` | DEMOLISH @R-01 | With the Domain model. |
| A-02 | `apps/api/src/index.ts` | EVOLVE @R-01… | R-01: drop the domains registration; later steps register v2 routes (media R-02, composer R-06, Stripe R-09). |
| A-03 | `apps/api/src/routes/observatories.ts` | EVOLVE @R-01 | Becomes API v2 core: drop `domainIds` validation; add `visibility`, content blocks, public world fetch by name, and the graph list (public worlds: name, displayName, visualSignature). |
| A-04 | `apps/api/src/routes/me.ts` | EVOLVE @R-01 | Same model changes on the owner routes; `name` stays immutable. |
| A-05 | `apps/api/src/lib/observatory-validation.ts` | EVOLVE @R-01 | Drop domain checks; add visibility + content-block validation. |
...
|---|---|---|---|
| P-01 | `Domain` model + `Observatory.domainIds` | DEMOLISH @R-01 | Migration #1. Nothing in v2 references domains. |
| P-02 | `apps/api/prisma/seed.ts` | EVOLVE @R-01 | Domain seed and v1 test observatories go; the new seed shape is decided at R-01. Production cleanup of already-seeded test rows = OQ-08. |
| P-03 | `Observatory` model | EVOLVE @R-01 | + `visibility` enum, + content JSONB; − `domainIds`; `publicMode` replaced by `visibility` (existing-row mapping = OQ-06). `userId @unique` and `name @unique` stand (R-DL-04/05). |
...
| ID | Path | Tag | Interim / notes |
|---|---|---|---|
| S-01 | `packages/shared/src/types/domain.ts` (+ its export in `types/index.ts`) | DEMOLISH @R-01 | |
| S-02 | `packages/shared/src/types/{system,publication}.ts` (+ exports) | DROP @R-10 | Mirrors of the P-04 tables; go when they go. |
| S-03 | `packages/shared/src/types/observatory.ts` | EVOLVE @R-01 | Gains `visibility` and content-block types; loses `domainIds`. |
...
| Step | Executes |
|---|---|
| **R-01** | W-01 (interim), W-03 (interim), W-04 (interim), W-08 (interim), W-11 (interim), W-13 (interim), W-14, W-15, W-16, W-27 (create), **W-02, W-28, A-01 (demolish)**, A-02, A-03, A-04, A-05, P-01 (demolish), P-02, P-03, **S-01 (demolish)**, S-03, C-01 (domain-copy spot fix) · migration #1 |
| **R-02** | A-07 (media presign, create), A-02, W-10 (image blocks), W-11 (interim upload wiring), P-03 (media shape in content blocks) |
